...
export async function reportRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/reports/revenue', async (request) => {
    ...
    return { total, count, byMonth: byMonth.rows.map((r) => ({ month: r.month, total: Number(r.total), count: Number(r.cnt) })) };
  });
  ...
  // c) Po majstoru
  app.get('/reports/mechanics/:id', async (request) => {
    ...
    return { mechanic: m.rows[0], orders: Number(rows[0]!.orders), hours: Number(rows[0]!.hours), value: Number(rows[0]!.value) };
  });

  // d) Po tipu vozila
  app.get('/reports/vehicle-types', async (request) => {
    const q = request.query as Record<string, string | undefined>;
    const conds: string[] = [];
    const params: unknown[] = [];
