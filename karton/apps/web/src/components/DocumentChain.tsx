 ...
 */
export function DocumentChainBar({ chain, currentId }: { chain: DocumentChain; currentId: number }): React.JSX.Element {
  const navigate = useNavigate();

  const cell = (label: string, ref: { id: number; number: string; status: string } | null, to: (id: number) => string) => {
    if (!ref) return <div className="chain-cell empty"><span className="chain-label">{label}</span><span className="chain-none">—</span></div>;
  ...
  return (
    <div className="chain-bar">
      <div className="chain-group">
        ...
        {cell('Ponuda', chain.quote, (id) => `/dokumenti/${id}`)}
