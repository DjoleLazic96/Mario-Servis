...
export async function dashboardRoutes(app: FastifyInstance): Promise<void> {
  app.get('/dashboard', { preHandler: requireAuth }, async () => {
    ...
    const [appts, waiting, inShop, openWO, pendingQ, monthRev, unpaid] = await Promise.all([
    ...
    return {
      ...
      business: { vehiclesInShop: Number(inShop.rows[0].n), openWorkOrders: Number(openWO.rows[0].n), pendingQuotes: Number(pendingQ.rows[0].n) },
